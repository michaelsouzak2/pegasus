...
def download(uid, username, password, outfile, show_progress=True, token=None, total_size=None):
    ...
    _download_raw_data(url, outfile, show_progress, total_size)
...
def _download_raw_data(url, outfile, show_progress, total_size=None):
    """Baixa dados da URL para outfile.incomplete e, em seguida, move para outfile"""
    outfile_temp = str(outfile) + ".incomplete"
    try:
        downloaded_bytes = 0
        with requests.get(url, stream=True, timeout=100) as req:
            # Adiciona o tamanho total do arquivo ao tqdm, se disponível
            if total_size:
                req.headers.get('content-length', total_size)
                
            with tqdm(total=total_size, unit="B", unit_scale=True, disable=not show_progress) as progress:
                chunk_size = 2**20  # baixar em chunks de 1 MB
                with open(outfile_temp, "wb") as fout:
                    for chunk in req.iter_content(chunk_size=chunk_size):
                        if chunk:  # filtrar novos chunks keep-alive
                            fout.write(chunk)
